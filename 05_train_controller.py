...
def initialize_settings(sigma_init=0.1, sigma_decay=0.9999, init_opt='', ROOT_DIR_NAME='./data'):
    global population, filebase, controller_filebase, model, num_params, es, PRECISION, SOLUTION_PACKET_SIZE, RESULT_PACKET_SIZE
    population = num_worker * num_worker_trial
    filebase = ROOT_DIR_NAME + '/log/'+env_name+'.'+optimizer + \
        '.'+str(num_episode)+'.'+str(population)
    controller_filebase = ROOT_DIR_NAME + '/controller/'+env_name+'.' + \
        optimizer+'.'+str(num_episode)+'.'+str(population)
    subprocess.run(['mkdir', '-p', filebase])
    subprocess.run(['mkdir', '-p', controller_filebase])

    model = make_model(ROOT_DIR_NAME)
...
def decode_solution_packet(packet):
    ...
    for p in packets:
        result.append(
            [p[0], p[1], p[2], p[3], p[4:].astype(np.float)/PRECISION])
    return result
...
def worker(weights, seed, max_len, new_model):
    ...
    reward_list, t_list = simulate(
        new_model, num_episode=num_episode, seed=seed, max_len=max_len
    )
...
def slave(ROOT_DIR_NAME):

    new_model = make_model(ROOT_DIR_NAME)
    dream_model = make_model(ROOT_DIR_NAME)
    ...
    while 1:
        ...
        assert(len(packet) == SOLUTION_PACKET_SIZE), (len(
...
def send_packets_to_slaves(packet_list, current_env_name, dream_mode):
    ...
    for i in range(1, num_worker):
        ...
        packet = {'result': packet,
                  'current_env_name': current_env_name, 'dream_mode': dream_mode}
...
def master(ROOT_DIR_NAME):
    ...
    while True:
        ...
        packet_list = encode_solution_packets(
            seeds, solutions, max_len=max_length)
        ...
        for current_env_name in config.train_envs:
            ...
            if len(config.train_envs) > 1:
                print('completed environment {} of {}'.format(
                    e_num, len(config.train_envs)))
            e_num += 1

        reward_list = reward_list / len(config.train_envs)
        time_list = time_list / len(config.train_envs)

        mean_time_step = int(np.mean(time_list)*100) / \
...
def main(args):
    ...
    initialize_settings(args.sigma_init, args.sigma_decay, init_opt, ROOT_DIR_NAME)

    sprint("process", rank, "out of total ", comm.Get_size(), "started")

    if (rank == 0):
        master(ROOT_DIR_NAME)
    else:
        slave(ROOT_DIR_NAME)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--init_opt', type=str, default='',
                        help='which optimiser pickle file to initialise with')
    parser.add_argument('-e', '--num_episode', type=int,
                        default=1, help='num episodes per trial (controller)')
    ...
    parser.add_argument('--max_length', type=int,
                        help='maximum length of episode', default=-1)
    ...
    parser.add_argument('--dream_mode', type=int,
                        help='train the agent in its dreams?', default=0)
