...
def make_model(ROOT_DIR_NAME='./data'):

    vae = VAE()
    vae.set_weights(ROOT_DIR_NAME + '/vae/weights.h5')

    rnn = RNN()
    rnn.set_weights(ROOT_DIR_NAME + '/rnn/weights.h5')
...
class Model:
    ...
    def make_env(self, env_name, seed=-1, render_mode=False, model=None):
        ...
        self.env = make_env(env_name, seed=seed,
                            render_mode=render_mode, model=model)
...
def main(args):
    ...
    if dream_mode:
        ...
        model.make_env(env_name + '_dream',
                       render_mode=render_mode, model=dream_model)
    else:
    ...
    if final_mode:
        ...
        for i in range(100):
            ...
            reward, steps_taken = simulate(
                model, num_episode=1, max_len=max_length, generate_data_mode=False)
            total_reward += reward[0]
            print("episode", i, "reward =", reward[0])
        print("seed", the_seed, "average_reward", total_reward/100)
    else:
        if record_video:
            subprocess.run(['mkdir', '-p', ROOT_DIR_NAME + '/videos'])
            model.env = Monitor(model.env, directory=ROOT_DIR_NAME + '/videos',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dream_mode', action='store_true',
                        help='run the model in the dreams of the agent')
